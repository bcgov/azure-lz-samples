...
$environmentNames = @($config.environment.Keys)
if (-not $config.environment.ContainsKey($environment)) {
  $availableEnvironments = $environmentNames -join ', '
  Throw "Environment '$environment' was not found in '$configFilePath'. Available environments: $availableEnvironments"
  Exit 1
}
$environmentConfig = $config.environment[$environment]
if (-not $environmentConfig.wiki -or $environmentConfig.wiki.Count -eq 0) {
  Throw "Environment '$environment' does not define any wiki entries in '$configFilePath'."
  Exit 1
}
foreach ($key in $environmentConfig.wiki.keys) {
  $wikiConfig = @{
    ...
    gitPlatform   = $environmentConfig.wiki[$key].gitPlatform
