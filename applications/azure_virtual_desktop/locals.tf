  ...
  session_host_instances = length(var.session_hosts) == 0 ? {} : merge([
    for session_host_key, session_host in var.session_hosts : {
      for index in range(coalesce(try(session_host.instance_count, null), local.session_host_defaults.instance_count)) : format("%s.%02d", session_host_key, index + 1) => {
        ...
        vm_name                      = format("%s-%02d", coalesce(try(session_host.vm_name_prefix, null), regexreplace(lower(session_host_key), "[^0-9a-z-]", "-")), index + 1)
        computer_name                = format("%s%02d", substr(coalesce(try(session_host.computer_name_prefix, null), regexreplace(lower(session_host_key), "[^0-9a-z]", "")), 0, 13), index + 1)
